...
namespace nil {
    namespace proof_verifier {
        namespace aspects {
            verifier_vanilla::verifier_vanilla(boost::shared_ptr<path> aspct) : path_aspect(std::move(aspct)) {
            }

            void verifier_vanilla::set_options(cli_options_type &cli) const {
            ...
            void verifier_vanilla::set_options(cfg_options_type &cfg) const {
            }

            void verifier_vanilla::initialize(configuration_type &vm) {
                if (vm.count("version")) {
#ifdef PROOF_VERIFIER_VERSION
#define xstr(s) str(s)
#define str(s) #s
        std::cout << xstr(PROOF_VERIFIER_VERSION) << std::endl;
#else
        std::cout << "undefined" << std::endl;
            ...
            boost::filesystem::path verifier_vanilla::default_config_path() const {
                return path_aspect->config_path() / "config.ini";
            }

            boost::filesystem::path  verifier_vanilla::input_circuit_file_path() const {
                return circuit_file_path;
            }

            boost::filesystem::path verifier_vanilla::input_assignment_file_path() const {
                return assignment_table_file_path;
            }

            boost::filesystem::path verifier_vanilla::output_proof_file_path() const {
                return proof_file_path;
            }

            detail::CurveType verifier_vanilla::curve_type() const {
                return curve_type_;
            }
